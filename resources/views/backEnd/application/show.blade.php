<x-app-layout>
    ...
    <a href="{{url()->previous()}}" class="btn btn-primary btn-sm mb-3"><i class="bx bx-arrow-back"></i> Back</a>
    @if($app->status ==='active')
        <a href="{{ route('application.statusUpdate', ['id' => $app->id, 'status' => 'deactive']) }}" class="btn btn-danger btn-sm mb-3"><i class='bx bx-x-circle'></i> Unqualified</a>
    @elseif($app->status ==='deactive')
        <a href="{{ route('application.statusUpdate', ['id' => $app->id, 'status' => 'active']) }}" class="btn btn-success btn-sm mb-3"><i class='bx bxs-badge-check'></i> Qualified</a>
    @endif
    </div>
    <div class="card">
